...
import { Activity, Zap, AlertTriangle, ArrowRight, Search, Filter, Radio } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    if (newlyArrived.size > 0 && previousTxIdsRef.current.size > 0) {
      ...
      const timer = setTimeout(() => {
        setNewTxIds((prev) => { const next = new Set(prev); newlyArrived.forEach((id) => next.delete(id)); return next; });
      }, 1500);
  ...
  const filteredTransactions = useMemo(() => {
    return [...transactions]
      .filter(tx => {
        ...
        if (riskFilter === 'FLAGGED' && !tx.reason) return false;
        if (searchQuery) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#080D18] overflow-hidden">
      ...
      <div className="flex items-stretch border-b border-[#1A2640] bg-[#040810] shrink-0">
        <div className="flex items-center gap-3 px-4 py-2.5 border-r border-[#1A2640]">
          <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse shrink-0" />
          <div>
            <div className="text-[8px] font-mono font-bold uppercase tracking-[0.14em] text-slate-700">Pipeline Status</div>
        ...
        {[
          ...
          { label: 'Critical Flags', value: criticalCount, unit: 'ALERTS', color: '#F87171' },
          { label: 'At-Risk Exposure', value: `₹${totalAtRiskAmount.toLocaleString('en-IN')}`, unit: '', color: '#FBBF24' },
        ].map((stat, i) => (
          <div key={i} className="flex flex-col justify-center px-5 py-2 border-r border-[#1A2640]">
      ...
      {/* ── Filter Bar ── */}
      <div className="flex items-center gap-2 px-4 py-2 border-b border-[#1A2640] bg-[#06091180] shrink-0">
        {/* Channel chips */}
        <div className="flex items-center gap-0.5">
          <span className="text-[8px] font-mono font-bold uppercase tracking-wider text-slate-700 mr-2">Channel</span>
        ...
        {/* Risk chips */}
        <div className="flex items-center gap-0.5">
          <span className="text-[8px] font-mono font-bold uppercase tracking-wider text-slate-700 mr-2">Severity</span>
          {[
            { id: 'ALL', label: 'All' },
        ...
        {/* Result count */}
        <span className="text-[9px] font-mono text-slate-700 ml-2 whitespace-nowrap">
          {filteredTransactions.length} rows
        </span>
      </div>

      {/* ── Table — edge-to-edge, no container ── */}
      <div className="flex-1 overflow-auto" style={{ background: '#080D18' }}>
        {filteredTransactions.length > 0 ? (
          <table className="s-table w-full min-w-[900px]">
            ...
            <tbody>
              {filteredTransactions.map((tx) => {
                ...
                const isFlagged = tx.regulatory_flagged || tx.reason;
                ...
                return (
                  <tr
                    ...
                    className={`${priorityClass} ${isNew ? 'animate-row-arrive' : ''} ${isCritical ? 'bg-rose-950/10' : ''}`}
                  >
                    {/* Priority dot */}
                    <td className="text-center px-2">
                      {isCritical && <span className="inline-block w-1.5 h-1.5 rounded-full bg-rose-500" />}
                    ...
                    {/* Time */}
                    <td className="text-slate-600 text-[10px]">{formatTime(tx.timestamp)}</td>

                    {/* Channel */}
                    <td>
                      <span className="inline-block text-[8px] font-bold px-1.5 py-0.5 rounded-sm bg-[#0F1926] text-slate-500 border border-[#1A2640] uppercase tracking-wider">
                        {tx.channel || 'UPI'}
                      </span>
                    </td>
                    ...
                    <td>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-blue-400">{role === 'admin' ? tx.receiver_account : maskAccount(tx.receiver_account)}</span>
                      </div>
                    </td>
                    ...
                    <td className="max-w-[240px]">
                      {tx.reason
                        ? <span className="text-amber-400/80 text-[10px] truncate block" title={tx.reason}>{tx.reason}</span>
                    ...
                    <td>
                      {tx.case_id
                        ? <span className="text-blue-500/70 text-[10px] font-mono">{String(tx.case_id).slice(-6)}</span>
          ...
          <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
            <div className="w-10 h-10 rounded bg-blue-600/8 border border-blue-500/15 flex items-center justify-center">
              <Radio className="w-5 h-5 text-blue-500/50 animate-pulse" />
            </div>
            <div>
              <div className="text-[11px] font-mono font-bold text-slate-500 uppercase tracking-wider">Monitoring Payment Networks</div>
              <div className="text-[10px] font-mono text-slate-700 mt-1">WebSocket: {connectionStatus}</div>
            </div>
          </div>
        )}
      </div>
